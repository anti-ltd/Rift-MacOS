...
ENTITLEMENTS     = Resources/Rift.entitlements
MAS_ENTITLEMENTS = Resources/Rift.mas.entitlements
MAS_PKG          = build/$(APP_NAME).pkg
MAS_PROFILE      ?= Resources/Rift_MAS.provisionprofile
...
SIGN_ID := $(shell security find-certificate -c "Rift Dev" >/dev/null 2>&1 && echo "Rift Dev" || echo -)

ifdef MAS
SWIFT_FLAGS += -Xswiftc -DRIFT_MAS
endif
...
DIST_DMG     = build/Rift-$(DIST_VERSION).dmg
DIST_JSON    = build/Rift-$(DIST_VERSION).json

dist: icon
	@echo "── Direct-distribution build: Rift $(DIST_VERSION) ──"
	...
	cp -R $(BUNDLE) build/dmg/
	ln -s /Applications build/dmg/Applications
	hdiutil create -volname "Rift" -srcfolder build/dmg -ov -format UDZO $(DIST_DMG)
	rm -rf build/dmg
...
dist-manifest:
	...
	NOTES=$${RIFT_RELEASE_NOTES:-"Initial release."}; \
	printf '{\n  "version": "%s",\n  "releasedAt": "%s",\n  "notes": "%s",\n  "minOS": "macOS %s",\n  "sha256": "%s",\n  "size": %d\n}\n' \
