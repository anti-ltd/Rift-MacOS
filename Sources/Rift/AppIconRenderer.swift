...
enum AppIconRenderer {
    ...
    private static func draw(in cg: CGContext, size: CGFloat) {
        ...
        let bgGrad = CGGradient(colorsSpace: space, colors: [
            rgb(0.48, 0.53, 0.98),   // #7A87FA — light blurple
            rgb(0.35, 0.40, 0.95),   // #5865F2 — Discord blurple
            rgb(0.20, 0.16, 0.68),   // deep indigo
        ...
        cg.setStrokeColor(rgb(1, 1, 1, 0.20))
        cg.strokePath()
        cg.restoreGState()
        ...
        cg.addPath(bg)
        cg.clip()
        cg.setShadow(offset: CGSize(width: 0, height: -size * 0.015), blur: size * 0.025,
